...
struct ClaudeInstancesView: View {
    ...
    private func focusSession(_ session: SessionState) {
        // Prefer Muxy's remote API when the session runs inside a Muxy pane —
        // this works without yabai and without any tmux-style process-tree
        // guesswork because Muxy gave us exact pane/project UUIDs via env vars.
...
struct InstanceRow: View {
    ...
    var body: some View {
        ...
        .onTapGesture {
            // Single-click anywhere on the row jumps to the terminal. Chat
            // remains available via the bubble icon — no double-tap handler
            // means the jump feels instant (no disambiguation delay).
            if session.isInMuxy || (session.isInTmux && isYabaiAvailable) {
                onFocus()
            }
        }
