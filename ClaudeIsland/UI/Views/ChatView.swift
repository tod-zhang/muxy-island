...
struct ChatView: View {
    ...
    /// Can send messages for tmux sessions (via tmux paste) or Muxy sessions
    /// (via Muxy's remote API terminalInput).
    private var canSendMessages: Bool {
        session.isInMuxy || (session.isInTmux && session.tty != nil)
    }

    private var inputBar: some View {
        HStack(spacing: 10) {
            TextField(canSendMessages ? "Message Claude..." : "Open Claude Code in tmux or Muxy to enable messaging", text: $inputText)
    ...
    private var interactivePromptBar: some View {
        ChatInteractivePromptBar(
            canGoToTerminal: session.isInMuxy || session.isInTmux,
    ...
    private func focusTerminal() {
        if let paneID = session.muxyPaneId, let projectID = session.muxyProjectId {
            Task {
                _ = await MuxyController.shared.focus(
                    paneID: paneID,
                    projectID: projectID,
                    worktreeID: session.muxyWorktreeId
                )
            }
            return
        }
    ...
    private func sendToSession(_ text: String) async {
        if let paneID = session.muxyPaneId {
            _ = await MuxyController.shared.sendInput(paneID: paneID, text: text)
            return
        }
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            Button {
                ...
                .foregroundColor(canGoToTerminal ? .black : .white.opacity(0.4))
                .padding(.horizontal, 16)
                .padding(.vertical, 8)
                .background(canGoToTerminal ? Color.white.opacity(0.95) : Color.white.opacity(0.1))
                .clipShape(Capsule())
            }
            .buttonStyle(.plain)
