...
def main():
    ...
    # here lets Vibe Notch drive Muxy's remote API directly without having
    # to reason about process trees or pane-content matching.
    muxy_pane_id = os.environ.get("MUXY_PANE_ID")
    muxy_project_id = os.environ.get("MUXY_PROJECT_ID")
    muxy_worktree_id = os.environ.get("MUXY_WORKTREE_ID")
    if muxy_pane_id:
        state["muxy_pane_id"] = muxy_pane_id
    if muxy_project_id:
        state["muxy_project_id"] = muxy_project_id
    if muxy_worktree_id:
        state["muxy_worktree_id"] = muxy_worktree_id
