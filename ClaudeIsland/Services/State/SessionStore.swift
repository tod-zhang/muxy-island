...
actor SessionStore {
    ...
    private func processHookEvent(_ event: HookEvent) async {
        ...
        // MUXY_* env vars are set at pane spawn and inherited by every child
        // process — so every hook event from a Muxy pane carries them. Copy
        // them in whenever present; never clear once learned (a subagent might
        // run outside the env but the root session still lives in the pane).
        if let paneId = event.muxyPaneId { session.muxyPaneId = paneId }
        if let projectId = event.muxyProjectId { session.muxyProjectId = projectId }
        if let worktreeId = event.muxyWorktreeId { session.muxyWorktreeId = worktreeId }
