...
struct SessionState: Equatable, Identifiable, Sendable {
    ...
    /// Muxy pane UUID, captured from MUXY_PANE_ID env var if the session
    /// runs inside a Muxy terminal pane. When present, the session can be
    /// focused via Muxy's remote WebSocket API without process-tree guesswork.
    var muxyPaneId: String?
    var muxyProjectId: String?
    var muxyWorktreeId: String?

    /// True when we have enough info to drive Muxy's remote API.
    var isInMuxy: Bool {
        muxyPaneId != nil && muxyProjectId != nil
    }
    ...
    nonisolated init(
        ...
        tty: String? = nil,
        isInTmux: Bool = false,
        muxyPaneId: String? = nil,
        muxyProjectId: String? = nil,
        muxyWorktreeId: String? = nil,
